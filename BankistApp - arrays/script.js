...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    inputLoginPin.blur(); //losing focus
...
const checkDogs = function (dogsJulia, dogsKate) {
  const correcteDogsJulia = dogsJulia.slice();
 ...
 */
/* 
// map method for arrays - returns new array updated by callback function
const eurToUsd = 1.1;
...
for (const move of movements) {
  moveUsdForOf.push(move * eurToUsd);
}
console.log(moveUsdForOf);

const movementsDescriptions = movements.map((move, i) => {
  return `Movement ${i + 1}: You ${
    move > 0 ? 'deposited' : 'withdrew'
  }withdrew ${Math.abs(move)}`;
});
...
const calcAverageHumanAge = function (ages) {
  const ageInHumanYears = ages.map(val => (val > 2 ? val * 4 + 16 : val * 2));
  const adultAges = ageInHumanYears.filter(age => age >= 18);
  const averageYear =
    adultAges.reduce((acc, act) => acc + act, 0) / adultAges.length;
...
// const allBalance = allMovements.reduce((acc, balance) => acc + balance, 0);
...
//Strings
const owners = ['Jonas', 'Adam', 'Zach', 'Marha'];
...
console.log(new Array(1, 2, 3, 45, 6, 5));
...
console.log(
  `Sarah dog is eating too ${
    sarahDog.recommendedFood > sarahDog.curFood ? 'little' : 'much'
  }`
);
